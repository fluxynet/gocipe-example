...
import (
	...
	"github.com/fluxynet/gocipe-example/models/citizen"
...
func Get(ctx context.Context, id string) (*Country, error) {
	...
	if rows.Next() {
		...
		default:
			...
			err = rows.Scan(entity.ID, entity.Name, entity.Continent, &capital_id, &citizen_id)
			if err != nil {
				return nil, err
			}

			entity.Capital, err = capital.Get(ctx, capital_id)
			if err != nil {
				return nil, err
			}
			entity.Citizen, err = citizen.Get(ctx, citizen_id)
			if err != nil {
				return nil, err
			}
		}
